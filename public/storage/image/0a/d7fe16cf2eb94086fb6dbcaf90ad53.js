...
(function (global, $) {
  ...
  var _util = {
    ...
    isQQLoginError: function () {
      return !!$.cookie("ywverify");
    },
    getUin: function () {
      var uin = $.cookie("uin");
      if (!uin) {
    ...
    sim2tra: function (obj) {
      if (
        typeof cnConvert !== "undefined" &&
        this.getUserLang() === _lang.zht
      ) {
    ...
    browser: {
      versions: (function () {
        ...
        return {
          ...
          qqLive: /QQLive(HD)?Browser[\s\/]*(\d+(\.\d+)*)/i.test(u),
        };
      })(),
    },
    ...
    debug: function (content) {
      ...
      if (!_debug_container) {
        ...
        $(document.body).append(
          '<div id="debugContainer" style="top:' +
            top +
            "px; left:" +
            left +
            'px; border:2px solid #ff0000;"></div>'
        );
...
(function ($) {
  ...
  function _compileTpl(tpl, data) {
    ...
    var lineTag = "\n",
      ...
      arrTpl = tpl
        .split(_left_separator)
        .join(_right_separator + logic_tag)
        .split(_right_separator),
      strCode = "var ",
      fn = null;
      ...
      for (var j = 0, count = arr_attr.length; j < count; j++) {
        ...
        if (2 == t.length) {
          if ("inner" == t[0]) {
            ...
            arr_bind.push({ el: el, event: t[0], action: t[1] });
          }
        }
      }
    }
...
(function ($) {
  ...
  var _maskCss = "maskUI",
    _win = $(window),
    _doc = $(document),
    _iframeSrc = /^https/i.test(window.location.href || "")
      ? "javascript:false"
      : "about:blank";
  ...
  function _center($el) {
    ...
    $el.css({
      ...
      top: (_util.getClientHeight() - $el.height()) / 2 + scrollTop + "px",
    });
  }
  CS.util.initNameSpace("CS");
    ...
    open: function () {
      ...
      if (_util.isIE6()) {
        this.layers.iframe = $(
          '<iframe class="' +
            _maskCss +
            '" style="background-color:#000; z-index:' +
            (zIndex - 1) +
            '; position:absolute; top:0; left:0; background:transparent; border:none; margin:0; padding:0; display:none;" src="' +
            _iframeSrc +
            '"></iframe>'
        );
      }
      ...
      if (
        typeof this.popup != "string" &&
        (this.popup.parentNode || this.popup.jquery)
      ) {
...
(function ($) {
  ...
  function _setEnv() {
    var currentUrl = window.location.href,
      releaseVersion = "0628162126",
      urlConfig = {
        dev: {
          siteUrl: "http://dev.chuangshi.qq.com/",
          cssUrl: "http://dev.sta1.chuangshi.qq.com/css/chuangshi/src/",
        },
        uat: {
          siteUrl: "http://uat.chuangshi.qq.com/",
          cssUrl:
            "http://uat.sta1.chuangshi.qq.com/chuangshi/" +
            releaseVersion +
            "/css/merge/",
        },
        release: {
          ...
          cssUrl:
            "http://sta1.chuangshi.qq.com/chuangshi/" +
            releaseVersion +
            "/css/merge/",
        },
      };
    var envTypeMatchs = currentUrl.match(/^http:\/\/(\w+)\./),
      envType =
        envTypeMatchs &&
        envTypeMatchs.length > 1 &&
        envTypeMatchs[1] in urlConfig
          ? envTypeMatchs[1]
          : "release";
...
(function ($) {
  ...
  _dialog.prototype = {
    _create: function () {
      var me = this;
      this.nodes = _uiBinder.appendHTML(
        document.body,
        _dialogTpl,
        { title: this.config.title, content: this.config.content },
        {
          close: function () {
            me.close();
          },
    ...
    addLeftMsg: function (msg) {
      if (msg) {
        $(this.nodes.buttonBox)
          .prepend('<span class="z xg1">' + msg + "</span>")
          .show();
      }
    },
    addButton: function () {
      if (
        ...
        this.nodes.buttonBox &&
        this.config.button &&
        this.config.button.length > 0
      ) {
        var me = this;
        $.each(this.config.button, function (index, item) {
          var input = document.createElement("input");
          input.type = "button";
          input.value = _util.sim2tra(item.value);
          input.className = "pn";
          $(input).css({
            padding: "0 10px",
            marginLeft: "10px",
  ...
  _Toast.prototype = {
    init: function () {
      $("#cstoastMessage").remove();
      var msgDIV = new Array();
      msgDIV.push('<div id="cstoastMessage">');
      ...
      var left =
        this.left == null
          ? (_util.getClientWidth() - msgEntity.find("span").width()) / 2 +
            scrollLeft +
            "px"
          : this.left;
      var top =
        this.top == null
          ? (_util.getClientHeight() - msgEntity.find("span").height()) / 2 +
            scrollTop +
            "px"
          : this.top;
      msgEntity.css({
  ...
  CS.dialog = {
    alert: function (content, callback, noticeType) {
      var title = _util.sim2tra("消息提示"),
        contentIcon = _contentIcon[noticeType]
          ? _contentIcon[noticeType]
          : _contentIcon["info"];
      ...
      return new _dialog({
        title: title,
        content: content,
        button: [{ value: "确定", focus: true }],
        beforeClose: callback,
      });
    },
    toast: function (message, time, left, top) {
      return new _Toast({ message: message, time: time, left: left, top: top });
    },
    confirm: function (content, ok, cancel) {
      ...
      return new _dialog({
        ...
        content: content,
        button: [
          { value: "确定", focus: true, callback: ok },
...
(function ($) {
  var _util = CS.util,
    ...
    _mask = CS.mask;
  var _dialogTpl = [
    '<div attr="inner:popup;" class="readPopup tipPopup" style="display:none;">',
    '<a attr="click:close;" class="closePopup" href="javascript:" title="关闭"></a>',
    '<div attr="inner:titleBox;" class="tipTitle">',
    "<h3><%=title%></h3>",
    "</div>",
    '<div class="tipContent"><%==content%></div>',
    '<center attr="inner:buttonBox;" class="tipBtnBox" style="display:none;"></center>',
    "</div>",
  ].join("");
  var _dialogDefaultConfig = {
    title: "消息提示",
...
(function ($) {
  ...
  var _params = {};
  function init(
    getLoginStatusAjaxUrl,
    sendYWLoginStatusAjaxUrl,
    logoutYWSsoAjaxUrl,
    checkTokenAjaxUrl,
    webSite
  ) {
    _params.getLoginStatusAjaxUrl = getLoginStatusAjaxUrl || "";
    _params.sendYWLoginStatusAjaxUrl = sendYWLoginStatusAjaxUrl || "";
    _params.logoutYWSsoAjaxUrl = logoutYWSsoAjaxUrl || "";
    _params.checkTokenAjaxUrl = checkTokenAjaxUrl || "";
    _params.webSite = webSite || "";
    loadYWssoJs();
    _listenMessageFromQQLoginIframe();
  }
  function loadYWssoJs() {
    var returnUrl = window.location.href;
    var ssoJsUrl = "";
    var tabshow = "111";
    ssoJsUrl = "https://sta.book.qq.com/js/ywlogin_v1.js";
    if (_env.envType !== "release") {
      tabshow = "111";
    }
    $.getScript(ssoJsUrl, function () {
      yw.setParams({
        appId: 1450000220,
        areaId: 1,
        returnUrl:
          _params.sendYWLoginStatusAjaxUrl +
          "?refererType=NORMAL&http_referer=" +
  ...
  function logout() {
    _util.request({
      url: _params.logoutYWSsoAjaxUrl,
      type: "GET",
      dataType: "json",
      success: function (json) {
        if (json && json.status) {
          if (json.data && json.data) {
            new Image().src = json.data;
          }
        }
      },
      complete: function () {
  ...
  function getLoginStatus() {
    if (!_params.getLoginStatusAjaxUrl || _util.isLogin()) {
      return;
    }
    _util.request({
      url: _params.getLoginStatusAjaxUrl,
      type: "GET",
      data: { site: _params.webSite },
      dataType: "jsonp",
      jsonp: "callback",
      jsonpCallback: "getStatus",
      success: function (json) {
        if (json && json.status) {
          if (json.data && json.data._ywtoken) {
            _checkToken(json.data._ywtoken);
          }
        } else {
        }
      },
  ...
  function _getYWssoToken(data) {
    if (!_params.sendYWLoginStatusAjaxUrl || _util.isLogin()) {
      return;
    }
    if (
      !data ||
    ...
    if (data.data.loginType === 20 && _util.isQQLoginError()) {
      return;
    }
    return;
  }
  function _checkToken(_ywtoken) {
    if (!_ywtoken) {
      return;
    }
    _util.request({
      url: _params.checkTokenAjaxUrl,
      data: { _ywtoken: _ywtoken },
      type: "GET",
      dataType: "json",
      success: function (json) {
        if (json && json.status) {
  ...
  function ywlogin_onResize(width, height) {
    var $loginPopup = $("#div_ywlogin"),
      $loginPopupIframe = $("#ui_ywlogin");
    ...
    if ($loginPopupIframe.length) {
      $loginPopupIframe.css({ width: width + "px", height: height + "px" });
    }
  }
  function _listenMessageFromQQLoginIframe() {
  ...
  CS.ssoLogin = {
    ...
    close: ywlogin_onClose,
    resize: ywlogin_onResize,
    getYWssoToken: _getYWssoToken,
    ssoLogout: ssoLogout,
  };
})(jQuery);
(function ($) {
  var _util = CS.util,
    _env = CS.config.env,
    _dialog = CS.dialog,
    _loginPopup = CS.ssoLogin;
  var _ajaxUrls = {
    addToBookshelf: "public/addToBookshelf",
    setAutoBuy: "booktake/setAutoBuy",
  };
  function addToBookshelf(bid, opts) {
    if (!bid) {
      return;
    }
    if (!_util.isLogin() && _loginPopup) {
      _loginPopup.showloginPopup();
      return;
    }
    opts = $.extend(
      {
    ...
    _util.request({
      url: _env.siteUrl + _ajaxUrls.addToBookshelf,
      data: { bid: bid },
      type: "POST",
      dataType: "json",
      success: function (json) {
        opts.successCallback(json);
      },
      complete: function () {
  ...
  function _setAutoBuy(bid) {
    if (!bid) {
      return;
    }
    _util.request({
      url: _env.siteUrl + _ajaxUrls.setAutoBuy,
      data: { bid: bid },
      type: "POST",
      dataType: "json",
      success: function (json) {
  ...
  _util.initNameSpace("CS");
  CS.bookshelf = { addToBookshelf: addToBookshelf };
})(jQuery);
(function ($) {
  var _util = CS.util,
    _env = CS.config.env,
  ...
  var _params = {},
    _nodes = null,
    _isSearchingBook = 0,
    _searchResultItemIndex = -1,
    _searchBookTimeout = null;
  var _ajaxUrls = {
    getUserInfo: "public/showheadv2",
    searchBook: "search/getPreSearchBooks",
    submitSearch: "Search/searchindex",
  };
  function init() {
    _getUserInfo();
  }
  function _getUserInfo() {
    _util.request({
      type: "GET",
      url: _env.siteUrl + _ajaxUrls.getUserInfo,
      dataType: "jsonp",
      jsonp: "callback",
  ...
  function _renderNavBar(userInfo) {
    _nodes = _uiBinder.appendHTML(
      $("#topNav"),
      $("#topNavBarTpl").val(),
      { isLogin: userInfo.isLogin === true, userInfo: userInfo },
      {
        openLoginPopup: _openLoginPopup,
        ywlogout: _logoutYWsso,
  ...
  function _bindEventBySearchInput() {
    if (!_nodes) {
      return;
    }
    var $searchInput = $(_nodes.searchInput),
      $searchResultBox = $(_nodes.searchResultBox),
      $searchResultList = $(_nodes.searchResultList);
    $searchInput
      .on("keyup", function (event) {
        switch (event.keyCode) {
          case 13:
            event.preventDefault();
            _submitSearch(event, this, _nodes);
            break;
          case 38:
            _selectSearchResult("up");
            break;
          case 40:
            _selectSearchResult("down");
            break;
          default:
            ...
            _searchBookTimeout = setTimeout(function () {
              _searchBookResult($searchInput.val());
            }, 200);
            break;
        }
      })
    ...
    $(_nodes.searchInputBox).on("mouseleave", function () {
      _closeSearchResultList();
    });
    $(document).on("click", function (e) {
      if ($(_nodes.searchInputBox).is(":visible")) {
        var $target = $(e.target);
        if (
          $target.closest([_nodes.searchInputBox, _nodes.showSearchInputBtnBox])
            .length === 0
        ) {
          _hideSearchInput();
          _closeSearchResultList();
  ...
  function _closeSearchResultList() {
    if (_nodes) {
      $(_nodes.searchResultBox).hide();
      $(_nodes.searchResultList).empty();
    }
  }
  function _showSearchInput(e, el, nodes) {
    $(nodes.showSearchInputBtnBox).hide();
    $(nodes.searchInputBox)
      .show()
      .animate({ width: "230px" }, 500, function () {
        $(nodes.searchInputBox).css("overflow", "");
      });
  }
  function _hideSearchInput() {
    if (_nodes && $(_nodes.searchInputBox).is(":visible")) {
      $(_nodes.searchInputBox).animate({ width: 0 }, 500, function () {
        $(_nodes.searchInputBox).hide();
        $(_nodes.showSearchInputBtnBox).show();
      });
    }
  }
  function _selectSearchResult(directionType) {
    if (!_nodes || $(_nodes.searchResultBox).is(":hidden")) {
      return;
    }
    var $items = $(_nodes.searchResultList).find("a");
    if (!$items || $items.length === 0) {
      return;
    }
    if (directionType === "up") {
      if (_searchResultItemIndex > 0) {
        _searchResultItemIndex--;
      }
    } else {
      if (_searchResultItemIndex < $items.length - 1) {
        _searchResultItemIndex++;
      }
    }
    var $selectedItem = $items.eq(_searchResultItemIndex);
    $items.removeClass("selected");
    $selectedItem.addClass("selected");
    $(_nodes.searchInput).val($selectedItem.text());
  }
  function _searchBookResult(searchValue) {
    if (!searchValue || $.trim(searchValue).length === 0) {
      return;
    }
    searchValue = _filterSearchWord(searchValue);
    ...
    _util.request({
      type: "GET",
      dataType: "json",
      url: _env.siteUrl + _ajaxUrls.searchBook,
      data: { keywords: searchValue },
      success: function (json) {
        if (json && json.status && json.data && json.data.html) {
          _searchResultItemIndex = -1;
          $(_nodes.searchResultList).html(json.data.html);
          $(_nodes.searchResultBox).show();
        }
      },
      error: function () {},
      complete: function () {},
  ...
  function _submitSearch(e, el, nodes) {
    var searchType = "all",
      searchValue = $.trim($(nodes.searchInput).val());
    searchValue = _filterSearchWord(searchValue);
    if (searchValue.length === 0) {
      _dialog.alert("请输入搜索内容");
      return false;
    }
    window.location.href =
      _env.siteUrl +
      _ajaxUrls.submitSearch +
      "?type=" +
      searchType +
      "&wd=" +
      searchValue;
  ...
  function _filterSearchWord(str) {
    var ret = "",
      strChar = "";
    for (var i = 0, len = str.length; i < len; i++) {
      strChar = str.charAt(i);
      if (/^[A-Za-z0-9\u4E00-\u9FFF：，]{1,15}$/.test(strChar)) {
        ret += strChar;
      }
    }
    return ret;
  ...
  function _openLoginPopup() {
    if (_loginPopup && typeof _loginPopup.showloginPopup === "function") {
      _loginPopup.showloginPopup();
    }
    return false;
  }
  function _logoutYWsso() {
    if (_loginPopup && typeof _loginPopup.ssoLogout === "function") {
      _loginPopup.ssoLogout();
    }
    return false;
  }
  _util.initNameSpace("CS");
  CS.topNavByRead = { init: init };
})(jQuery);
(function ($) {
  var _util = CS.util,
    _env = CS.config.env;
  var _params = {};
  var _cmreadUrl =
    "http://wap.cmread.com/r/447389415/447389723/index.htm?cm=M33W0001";
  function init(
    commonResourceUrl,
    refreshKeyUrl,
    requestStatLogUrl,
    ...
    _params.commonResourceUrl = commonResourceUrl || "";
    _params.refreshKeyUrl = refreshKeyUrl || "";
    _params.requestStatLogUrl = requestStatLogUrl || "";
    _params.requestStatLogData = requestStatLogData || {};
    _initUserBehaviorLog();
    _requestStatLog();
    _requestCmReadLog();
  }
  function _requestCmReadLog() {
    new Image().src = _cmreadUrl;
  }
  function _requestStatLog() {
    var params = [],
      url = "";
    $.each(_params.requestStatLogData, function (name, val) {
      params.push(name + "=" + encodeURIComponent(val));
    });
    url = _params.requestStatLogUrl + "?" + params.join("&");
    if (document.referrer) {
      url += "&ref=" + encodeURIComponent(document.referrer);
    }
    new Image().src = url;
  }
  function _initUserBehaviorLog() {
    $.getScript(
      _params.commonResourceUrl + "/statistics/user.min.js",
      function () {
